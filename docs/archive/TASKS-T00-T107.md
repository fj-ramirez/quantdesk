> **Archived 2026-09-22 — frozen.** This is the complete task history T00–T107, moved here
> from the repo root unchanged except that relative links were re-rooted (`../../`) so they
> still resolve. Open work lives in [`TASKS.md`](../../TASKS.md); the durable decisions these
> blocks established are indexed in [`context/decisions.md`](../../context/decisions.md).
> Code comments that cite "TASKS.md Txx" for a finished task refer to this file. Status markers
> below are as they stood when frozen — several are stale; `context/decisions.md`'s ledger and
> `plans/audit-and-compaction/README.md` record the corrections. Do not edit; add an addendum
> at the end if history must be corrected.
...
Corrections recorded when this file was frozen. The blocks above are left as they stood; where
they and this addendum disagree, this addendum is right. Evidence: `git log`, the plan files'
*Result* headings and the code, cross-checked in
[plans/audit-and-compaction/README.md](../../plans/audit-and-compaction/README.md).

## T108 · Opus · T77 (allocated retroactively)

**Score the paper watchlist, and keep the scores.** Shipped 2026-09-20 as commit `808dee9`,
whose subject says "T83" — a collision with the unrelated, still-open T83 block above.
`research.paper_scores` holds one row per candidate per cycle, keyed on `(hash, scored_at)` and
never updated, because the trajectory is the evidence and last-writer-wins would erase a
...
- **Done, no Done marker above:** T00–T14, T16, T27, T29, T30, T34–T41, T59 (all merged, per
  `git log`); T37 shipped inside the T39/T40 commit `7bda7a7`; T105 shipped with T106
  (`be94b89`, `eb10e3b`).
- **Deployed, though marked "deploy pending" / "not yet deployed":** T87, T88 (later homeserver
  deploys include them), T104 (commit `858463e` was "found by deploying it"). T100 and T103 are
  done; their deployment is not separately recorded.
- **T89 — closed, not needed.** The capture-memory gate answered "retention" (commit `0fc1d9b`).
- **T70 — superseded.** The "no git remote" and "stay on the laptop" statements are stale:
  `origin` exists and a homeserver runs production. Its one surviving requirement — the desk is
  never exposed publicly — is in `context/decisions.md`.
- **T46** folded into T44; **T15**'s first bullet superseded (prices come from `daily_bars`).
- **T73** done; its open residual is now **T110**. **T74** done; its UI follow-on is **T111**.
  T47's unnumbered safety-net TODO is **T109**; T61's calibration ("T62 candidate") is **T112**;
  T92's `variance_ratio` finding is **T113**.
- **Still open, carried to `TASKS.md`:** T17, T20, T25, T28 (partial), T31, T33, T57, T58, T72,
  T81 (partial), T83, T84, T85, T94, T95, T96; blocked T21–T24; conditional T26.
- **Unverified acceptance items, still unverified:** T03 (never run against the real API), T82
  (a second non-Claude client; `claude mcp list`), T86 (capture timing and memory under the cap),
  T106 (the call-count measurement).
