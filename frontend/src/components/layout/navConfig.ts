 ...
 *
 * Route paths are exactly T62's inventory (`plans/ui-ux-refresh/01-ux-baseline.md`'s
 * "Navigation mapping" table) — only labels and grouping changed. "Opportunities" and
...
export interface NavItem {
  ...
  label: string;
  to: string;
  /** Passed straight through to `NavLink`'s own `end` prop — only `/` needs it, since every
   * other route is not a prefix of any other route in this list. */
  end?: boolean;
}
...
export const NAV_GROUPS: readonly NavGroup[] = [
  {
    ...
    entries: [
      ...
      { key: 'decisions', label: 'Opportunities', to: '/decisions' },
    ],
  },
  {
    title: 'Analyze',
    entries: [
      { key: 'dashboard', label: 'GEX Explorer', to: '/dashboard' },
      {
        pair: [
          { key: 'regime', label: 'Regime', to: '/regime' },
          { key: 'scan', label: 'Scan', to: '/scan' },
        ],
      },
      {
        pair: [
          { key: 'rotation', label: 'Rotation', to: '/rotation' },
 ...
 * target IA diagram. */
export const SETTINGS_NAV_ITEM: NavItem = { key: 'settings', label: 'Settings', to: '/settings' };
