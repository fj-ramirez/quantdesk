...
describe('BarsFreshness', () => {
  ...
  it('shows the stale count when non-zero', async () => {
    server.use(http.get('*/api/health/capture', () => HttpResponse.json(healthCaptureStaleFixture)));
    renderBarsFreshness();
  ...
  it('renders an unavailable message on a transport failure, never a raw body', async () => {
    server.use(http.get('*/api/health/capture', () => HttpResponse.json({ detail: 'boom' }, { status: 500 })));
    renderBarsFreshness();
