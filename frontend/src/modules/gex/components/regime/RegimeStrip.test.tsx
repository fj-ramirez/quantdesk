...
describe('RegimeStrip', () => {
  ...
  it('renders "n/a" tiles with a reason in the tooltip when nothing is seeded yet', async () => {
    server.use(http.get('*/api/gex/scan/cross-asset', () => HttpResponse.json(crossAssetEmptyFixture)));
    renderStrip();
    await waitForLoaded();
  ...
  it('renders an unavailable message on a transport failure, never a raw body', async () => {
    server.use(http.get('*/api/gex/scan/cross-asset', () => HttpResponse.json({ detail: 'boom' }, { status: 500 })));
    renderStrip();
