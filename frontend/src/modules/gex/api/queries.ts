...
// `bars` block -- one hook per `app/modules/gex/api/scan.py` / `app/modules/gex/api/bars.py` / `app/modules/gex/api/health.py`
// route, same "components call hooks, never `apiClient` directly" rule as above.
//
// No caching tricks beyond TanStack Query's own defaults: `GET /api/gex/scan/trend` genuinely
