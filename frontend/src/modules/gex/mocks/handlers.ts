...
// `GET /api/gex/report/{underlying}` response body, produced by running a real Cboe chain through
 ...
 * `app/modules/gex/gex/report.py` is the source of truth; this only has to reproduce the same *shape* and
 ...
 * every sector/industry ETF today. That last case mirrors `app/modules/gex/api/report.py`'s degrade: no
...
export const handlers = [
  http.get('*/api/gex/gex/:underlying/latest', ({ params, request }) => {
  ...
  http.get('*/api/gex/gex/:underlying/levels/history', ({ params, request }) => {
    const underlying = String(params.underlying).toUpperCase();
    if (!isUnderlying(underlying)) return notFound(`unknown underlying ${underlying}`);
  ...
  http.get('*/api/gex/chains/:underlying/latest', ({ params, request }) => {
    const underlying = String(params.underlying).toUpperCase();
    if (!isUnderlying(underlying)) return notFound(`unknown underlying ${underlying}`);
  ...
  http.get('*/api/gex/report/:underlying', ({ params, request }) => {
    const underlying = String(params.underlying).toUpperCase();
  ...
  http.post('*/api/gex/snapshots/capture', ({ request }) => {
  ...
  http.get('*/api/gex/snapshots', ({ request }) => {
    const url = new URL(request.url);
  ...
  // block, and `/api/gex/symbols`. `breakouts.json`/`trend.json` are the live, unedited universe
  ...
  http.get('*/api/gex/scan/breakouts', ({ request }) => {
    const url = new URL(request.url);
  ...
  http.get('*/api/gex/scan/breakouts/:symbol', ({ params, request }) => {
    ...
    // Mirrors `app/modules/gex/api/scan.py`'s "never 404s" contract: a symbol with no stored bars
    // returns a clean, empty events list.
    return HttpResponse.json({ symbol, n, k, lookback, events: [] } satisfies SymbolBreakoutsResponse);
  }),

  http.get('*/api/gex/scan/trend', () => HttpResponse.json(trendFixture as TrendResponse)),

  http.get('*/api/gex/scan/trend/:symbol', ({ params }) => {
    const symbol = decodeURIComponent(String(params.symbol)).trim().toUpperCase();
    if (symbol === 'SPY') {
  ...
  http.get('*/api/gex/bars/:symbol', ({ params }) => {
  ...
  http.get('*/api/gex/universe', () => HttpResponse.json(universeFixture as UniverseResponse)),
  ...
  http.get('*/api/gex/scan/rotation', ({ request }) => {
    const url = new URL(request.url);
  ...
  http.get('*/api/gex/scan/regime', ({ request }) => {
    const url = new URL(request.url);
  ...
  http.get('*/api/gex/health/capture', () => HttpResponse.json(healthCaptureFixture as CaptureHealth)),
  ...
  http.get('*/api/gex/scan/flows', ({ request }) => {
    const url = new URL(request.url);
  ...
  http.get('*/api/gex/scan/cross-asset', () =>
  ...
  http.get('*/api/gex/decisions', ({ request }) => {
    const url = new URL(request.url);
