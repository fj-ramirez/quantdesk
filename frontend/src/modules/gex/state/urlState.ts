...
/** `/scan`'s `N` toggle -- `app/modules/gex/api/scan.py`'s `_VALID_N`. */
export const SCAN_N_VALUES = [20, 55] as const;
export const DEFAULT_SCAN_N = 20;

/** `/scan`'s `k` toggle -- `app/modules/gex/api/scan.py`'s `_VALID_K`. */
export const SCAN_K_VALUES = [3, 5, 10] as const;
export const DEFAULT_SCAN_K = 5;

/** `/scan`'s lookback toggle. `app/modules/gex/scan/breakouts.py`'s `DEFAULT_LOOKBACK` (126) is the
