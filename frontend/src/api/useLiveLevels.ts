 ...
 * removes the choice.
 *
 * The event is a **nudge, not a payload** (see `app/api/stream.py`): it carries enough to show a
...
/** Shape of the `levels` event's JSON payload. Mirrors `app/api/stream.py`'s `_sse` call. */
...
export function useLiveLevels(underlying: Underlying): {
  ...
  useEffect(() => {
    ...
    const source = new EventSource(new URL(`/api/stream/${underlying}`, API_BASE_URL).toString());
