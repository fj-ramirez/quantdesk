"""Tests for `GET /api/gex/symbols` (T47) -- the core/extended universe split at the wire.

Entirely offline and trivial on purpose: `app/modules/gex/api/symbols.py` does no I/O, so these tests only
...
from app.modules.gex.api.symbols import router
...
def test_symbols_returns_core_and_extended_lists():
    response = _client().get("/api/gex/symbols")
