"""Tests for `GET /api/gex/scan/flows` (T52's additive block in `app/modules/gex/api/scan.py`). Kept as its
...
Offline: `app.modules.gex.storage.flows_repository.get_session_factory` is monkeypatched directly (the
route imports it locally inside `get_flows`, not as a module-level name in `app.modules.gex.api.scan`), no
real Postgres.
"""
...
def test_get_flows_rejects_an_invalid_window(client, session_factory):
    response = client.get("/api/gex/scan/flows", params={"window": 7})
    assert response.status_code == 422


def test_get_flows_defaults_to_window_20(client, session_factory):
    response = client.get("/api/gex/scan/flows")
...
def test_get_flows_lists_unsupported_symbols_as_no_flow_data_never_a_zero_bar(
    ...
    """
    response = client.get("/api/gex/scan/flows", params={"window": 5})
    body = response.json()
...
def test_get_flows_reports_history_since_message_when_window_exceeds_available_history(
    ...
    # Anchored via `app.modules.gex.api.scan._today` (same convention `test_scan_api.py`'s rotation tests
    ...
    response = client.get("/api/gex/scan/flows", params={"window": 20})
    body = response.json()
