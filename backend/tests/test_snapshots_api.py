"""Tests for `app/modules/gex/api/snapshots.py`. Offline: `capture_snapshot` and `get_session_factory`
are monkeypatched at their import site in `app.modules.gex.api.snapshots`, so no network or real Postgres
...
from app.modules.gex.api.snapshots import router
...
def test_capture_rejects_unsupported_underlying_without_calling_capture(client, monkeypatch):
    ...
    monkeypatch.setattr("app.modules.gex.api.snapshots.capture_snapshot", fake_capture_snapshot)

    response = client.post("/api/gex/snapshots/capture", params={"underlying": "DOGE"})
...
def test_capture_success_returns_201_with_result_body(client, monkeypatch):
    ...
    response = client.post("/api/gex/snapshots/capture", params={"underlying": "SPX", "eod": "false"})
...
def test_capture_failure_returns_502(client, monkeypatch):
    ...
    response = client.post("/api/gex/snapshots/capture", params={"underlying": "SPX"})
...
def test_capture_passes_eod_flag_through(client, monkeypatch):
    ...
    client.post("/api/gex/snapshots/capture", params={"underlying": "spy", "eod": "true"})
...
def test_list_snapshots_returns_rows_newest_first(client, monkeypatch, session_factory):
    monkeypatch.setattr("app.modules.gex.api.snapshots.get_session_factory", lambda: session_factory)
    ...
    response = client.get("/api/gex/snapshots", params={"underlying": "SPX", "limit": 30})
...
def test_list_snapshots_respects_limit(client, monkeypatch, session_factory):
    ...
    response = client.get("/api/gex/snapshots", params={"underlying": "SPX", "limit": 2})
...
def test_list_snapshots_empty_for_unknown_underlying(client, monkeypatch, session_factory):
    monkeypatch.setattr("app.modules.gex.api.snapshots.get_session_factory", lambda: session_factory)
    response = client.get("/api/gex/snapshots", params={"underlying": "QQQ"})
    assert response.status_code == 200
