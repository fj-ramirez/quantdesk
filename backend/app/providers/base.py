...
:class:`~app.models.chain.ChainSnapshot`, so the contract below is deliberately narrow.

What a provider is responsible for
----------------------------------

1. **Fetching** the vendor's raw chain for one underlying (network, auth, retries, backoff).
2. **Normalizing** it into :class:`~app.models.chain.ChainSnapshot` using
   :func:`~app.models.chain.parse_occ_symbol` and
   :meth:`~app.models.chain.OptionContract.from_occ`, so that root, underlying, expiry,
...
class OptionChainProvider(ABC):
    ...
    def delayed_minutes(self) -> int:
        ...
        Copied onto every :class:`~app.models.chain.ChainSnapshot` this provider returns, and
