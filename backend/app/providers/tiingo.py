...
symbols via ``BAR_PROVIDER_GROUPS``, see `app.providers.bars`), mirroring how
``app.providers.marketdata.MarketDataProvider`` is Cboe's break-glass fallback: gated behind a
...
verified against a live token before being trusted the way `app.providers.yahoo` now is.
...
from app.providers.bars import BarProvider, ProviderError, SymbolNotSupported, UpstreamUnavailable
...
class MissingCredential(ProviderError):
    ...
    `app.providers.marketdata.MissingCredential`: a blank token header is a confusing vendor
    ...
    async def _fetch_json(self, url: str, *, requested_symbol: str) -> Any:
        ...
        Raises:
            SymbolNotSupported: a 404, which Tiingo's docs describe as "ticker not found."
                Unlike Yahoo's 404 (module docstring, `app.providers.yahoo`), this has not been
...
if __name__ == "__main__":
    ...
    if len(sys.argv) not in (2, 3):
        print("usage: python -m app.providers.tiingo SYMBOL [years]", file=sys.stderr)
        raise SystemExit(2)
