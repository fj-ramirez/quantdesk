...
* **Unknown symbol.** HTTP **404** with a structured body: ``chart.result`` is ``null`` and
  ...
  delisted"}``. Mapped to :class:`~app.providers.bars.SymbolNotSupported`, not to a rate-limit
  error. A non-200 whose body is not that shape, and any 429, maps to
  :class:`~app.providers.bars.UpstreamUnavailable`. Re-confirmed live 2026-09-09 against a
...
* ``^VIX`` **volume is always 0.** Treated as nullable throughout this app
  (``app.models.bars.DailyBar.volume``), and a zero here is passed through unchanged rather
...
* **SPX is** ``^GSPC`` **on Yahoo**, not ``^spx``. ``^SPX`` is not a Yahoo symbol. See
  ...
  see `app.providers.bars.BarProvider.fetch_daily_bars`'s own docstring for why that boundary
...
``app.providers.tiingo``, the token-guarded fallback). It is exactly why
`app.providers.bars.BarProvider` is not optional: when Yahoo goes the way Stooq just did (dead
...
* **Parallel nulls.** A halted or untraded day can leave `null` in the same index position
  ...
  is `null` is skipped entirely, never stored as `0.0` — `app.models.bars.DailyBar`'s price
...
* **A 200 whose body is not JSON** (a CDN error page or bot-challenge HTML, the same failure
  mode `app.providers.cboe` guards against) raises `UpstreamUnavailable` rather than letting a
...
# user agent -- a browser-like one is cheap insurance, same rationale as app.providers.cboe.
_USER_AGENT = (
...
# admitted -- mirrors the spirit of app.jobs.catchup.EOD_CUTOFF's 16:20 grace. Exists so a
...
def _at(values: list[Any] | None, i: int) -> Any:
    ...
    shape as `app.providers.marketdata._at`)."""
...
class YahooBarProvider(BarProvider):
    ...
    async def _fetch_chart(self, url: str, *, requested_symbol: str) -> dict[str, Any]:
        ...
        raise `UpstreamUnavailable`, same shape as `app.providers.cboe`'s `_fetch_json`.
        """
    ...
    def _parse_payload(
        ...
        for i, ts in enumerate(timestamps):
            ...
            try:
                ...
                # unknown-root-style policy as app.providers.cboe: one bad row must not blank
                # out the whole fetch.
                skipped_null += 1
                logger.warning(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) not in (2, 3):
        print("usage: python -m app.providers.yahoo SYMBOL [years]", file=sys.stderr)
        raise SystemExit(2)
