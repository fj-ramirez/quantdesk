...
Pure functions over a :class:`~app.models.chain.ChainSnapshot` (or the ``DataFrame`` built
...
* ``gamma`` — spot gamma, ∂²V/∂S², **unsigned**, from :mod:`app.gex.greeks` recomputed at
...
:mod:`app.gex.greeks` documents; pre-flattening to ``(M, N)`` measures about 2× slower. The
...
from app.gex import greeks
from app.models.chain import ChainSnapshot, Right, Settlement
...
def to_frame(snapshot: ChainSnapshot, *, now: dt.datetime | None = None) -> pd.DataFrame:
    """Flatten a :class:`~app.models.chain.ChainSnapshot` into the engine's working frame.
