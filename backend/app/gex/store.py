...
and the GEX engine (T08). ``app.gex.engine`` is pure and does no I/O by design; this module is
...
* **Every level column is nullable and every write here preserves exactly what
  ...
  docstring in ``app.models.db`` for why (the ZERO_DTE-after-close case is the everyday
...
* **A level-computation failure must never cost the underlying capture.** This function
  ...
  swallowing it -- that decision belongs to the caller. ``app.jobs.capture``'s T09 seam and
  the backfill CLI (``app.gex.backfill``) each catch, log, and move on, exactly because the
...
logger = logging.getLogger("app.gex.store")
...
# A cache separate from `app.jobs.capture`'s own: importing that module's factory here would
# be a circular import (the T09 seam in capture.py calls into this module), and its docstring
# already says not to reuse it outside the capture path. `app.jobs.capture` passes its own
...
def get_session_factory() -> sessionmaker[Session]:
    ...
    same pattern as `app.jobs.capture.capture_snapshot`.
    """
...
def compute_and_store(
    ...
    *,
    ...
    `app.gex.engine.to_frame` (shared across every filter -- the flatten, not the aggregation,
    ...
    Args:
        ...
        session_factory: Defaults to `get_session_factory()` (real Postgres). Tests, and
            `app.jobs.capture`'s capture hook, pass an explicit factory.
