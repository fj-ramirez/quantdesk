...
rate and dividend yield, resolved from :mod:`app.config` at call time when the caller passes
``None``.
...
from app.models.chain import Settlement
...
def time_to_expiry(
    ...
    *,
    ...
    Args:
        ...
        settlement: A :class:`~app.models.chain.Settlement` (or its ``"AM"``/``"PM"`` string),
            or an array of them.
...
def call_put_sign(right: Any) -> np.ndarray | np.float64:
    ...
    Accepts :class:`~app.models.chain.Right` members, ``"C"``/``"P"`` (any case, and any word
