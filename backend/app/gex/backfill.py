...
Run as::
    ...
    uv run python -m app.gex.backfill

Idempotent by the same construction as `app.gex.store.compute_and_store`: a snapshot counts
as "lacking levels" only when it does not already carry a `gex_levels` row for every filter in
`app.gex.store.DEFAULT_FILTERS`, so a second run against an unchanged database finds nothing
...
from app.gex.store import DEFAULT_FILTERS, compute_and_store, get_session_factory
from app.models.db import GexLevel, Snapshot

__all__ = ["backfill", "main"]

logger = logging.getLogger("app.gex.backfill")
