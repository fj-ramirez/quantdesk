...
Run as::
    ...
    uv run python -m app.bars_backfill --years 5 [--symbols SPY,QQQ,SPX] [--sleep 0.5] [--force]

Mirrors `app.gex.backfill`'s shape (idempotent, summary dict, non-zero exit on any failure) but
is its own top-level module rather than living under `app.gex` -- CLAUDE.md invariant 1 keeps
`app/gex/*` reserved for the pure Greeks/engine code path, and bars have nothing to do with
...
unthrottled-by-contract endpoint (`app.providers.yahoo`) rather than a paid API with a
...
from app.providers.bars import BarProviderRegistry, ProviderError
from app.storage.bars_repository import get_session_factory, last_bar_date, upsert_bars

__all__ = ["backfill", "main"]

logger = logging.getLogger("app.bars_backfill")
...
#: window (`app.jobs.bars`), so the backfill CLI and the scheduled job agree on "up to date."
    ...
    try:
        for i, symbol in enumerate(active_symbols):
            ...
            try:
                ...
                # Same rationale as app.jobs.bars.update_one_symbol: a provider bug leaking
