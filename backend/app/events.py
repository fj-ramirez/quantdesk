...
`publish` is called from `app.jobs.capture.capture_snapshot`, which is `async` and runs on the
...
#: The application's broker. Imported by `app.jobs.capture` (publisher) and `app.api.stream`
