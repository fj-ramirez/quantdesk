...
Mirrors `app.storage.bars_repository`'s shape (plain module-level functions, a cached
...
from app.models.db import EtfSharesOutstanding, get_engine, get_sessionmaker
...
# app.storage.bars_repository's own module-level cache.
...
def insert_new_rows(
    ...
    This is what makes the T52 acceptance check literal: `python -m app.flows_fetch
...
def _read_universe_wide(
    ...
    *,
    ...
    `app.scan.flows.compute_flows` can index `frame[symbol]` unconditionally the same way every
...
def read_universe_shares_outstanding(
    ...
    *,
    ...
    `symbols` -- the `so_frame` shape `app.scan.flows.compute_flows` expects."""
...
def read_universe_nav(
    ...
    *,
    ...
    `symbols` -- the `nav_frame` shape `app.scan.flows.compute_flows` expects."""
    return _read_universe_wide(
