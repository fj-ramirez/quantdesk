...
(the bars job, the backfill CLI, `/api/bars`, and every later continuation-plan scan tool that
...
defaults to this module's own cached sessionmaker (real Postgres, `app.config.settings`), same
DI pattern as `app.jobs.capture.get_session_factory` -- so tests inject a temp-SQLite factory
...
# app.jobs.capture._session_factory: creating a fresh engine per call would open (and never
