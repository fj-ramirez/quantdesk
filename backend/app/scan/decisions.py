...
top of `app.scan.regime` (dealer positioning, walls, flip, ATR-normalized distances and the
fade/continuation verdict), `app.scan.trend` (the cross-sectional trend composite) and
`app.scan.breakouts` (the historical continuation base rate), and adds no data source of its
own -- every number it emits is a level, a distance or a rate one of those modules measured.

**Pure, on exactly the same contract as `app.gex.engine`, `app.scan.regime` and friends**: no
HTTP, no database, no filesystem, no logging, no clock. `app.api.decisions` is the only caller
that does I/O; it builds the inputs the same way `app.api.scan.get_regime` does and hands them
...
Scope, unchanged from PLAN.md section 2 and `app.gex.report`'s "Trade suggestions" section:
...
Two setups, mirroring the two verdicts `app.scan.regime` can reach:
...
never a passing one either" posture `app.scan.regime._verdict` takes for the 0DTE share. Grades:
...
from app.gex.engine import StrikeGex
from app.scan.breakouts import BreakoutSummary, Direction, Outcome
...
#: be tagged in the next session. Deliberately wider than `app.scan.regime.FADE_WALL_ATR`
...
def _ladder(
    ...
    *,
    ...
    `start` first. The same "comparable gamma" threshold `app.scan.regime._room_beyond` uses,
    ...
    *,
    ...
    if ahead is not None:
        ...
        if wall_pays:
            ...
            # the next strike with comparable gamma, which `app.scan.regime` already found.
...
def decide(
    ...
    *,
    ...
    Args:
        regime: The symbol's `app.scan.regime.RegimeRow` -- spot, ATR, walls, flip, verdict.
        ...
        trend_composite: `app.scan.trend.TrendRow.composite` for this symbol, a 0-1 percentile
            across the universe, or `None` when it has no bars history.
        breakouts: `app.scan.breakouts.BreakoutSummary` for this symbol, or `None`.
    ...
    if not reasons:
        ...
        if verdict == "continuation":
            ...
            # Short gamma always reads `continuation` in `app.scan.regime._verdict`; a `None`
