...
literal lists, split out of `app.scan.rotation` into their own tiny module because the plan
...
it, and a future `app.scan.rotation` change must never silently change what "sectors" means to
those other tools.
...
import `app.config` to re-derive that membership at runtime. Two reasons: (1) purity -- this
package (`app.scan.*`) must stay import-free of anything that pulls in `pydantic_settings`
machinery transitively, the same "no HTTP, DB, filesystem or logging" contract every other
`app.scan` module states for itself; (2) stability -- `SCAN_UNIVERSE` is a single flat string a
...
changed shape. If a member is ever removed from `SCAN_UNIVERSE` entirely, `app.storage
...
* **`BENCHMARKS`** -- the plan's two benchmark choices for the rotation math: `SPY`
  ...
  toggle -- also the equal-weight leg of `app.scan.rotation.sector_breadth`'s own RSP/SPY
...
#: `GET /api/scan/rotation?group=` accepts exactly these three names -- the API layer's 422
#: validation (`app.api.scan._validate_group`) rejects anything else, mirroring how `n`/`k`
...
#: `app.scan.rotation.sector_breadth` reads to measure equal- vs cap-weight leadership.
