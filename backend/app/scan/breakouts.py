...
history that already happened. **Pure, exactly like `app.gex.engine`**: no HTTP, DB, filesystem
or logging. Data comes in as a `pd.DataFrame` of daily bars (the shape `app.storage
.bars_repository.read_bars` returns -- see that module and `app.scan.indicators` for the exact
column contract), results go out as frozen dataclasses. `app.api.scan` does all the I/O:
...
* **Follow-through** = `sign · (close[t+k] − level) / ATR14[t]`, `None` once resolved only if
  `ATR14[t]` itself is unmeasurable (insufficient history -- see `app.scan.indicators`) or
...
from app.scan.indicators import ATR_PERIOD, atr
...
class Direction(StrEnum):
    ...
    def sign(self) -> int:
        ...
        by, mirroring `app.gex.engine`'s dealer-sign convention (+calls/-puts) in spirit: the
...
#: (`pandas.bdate_range`) since this module cannot import `app.jobs.calendar` (the purity
...
#: holiday noise, because the two look the same size. `app.api.scan` (which does I/O already,
#: and is not bound by this module's purity contract) does this check itself instead, using
#: `app.jobs.calendar.is_trading_day` for an exchange-holiday-aware expected count -- see that
