...
Answers a different question than `app.scan.breakouts`: not "how have past breakouts played
out" but "what does the *current* tape look like, right now, across the universe." **Pure,
exactly like `app.gex.engine` and `app.scan.breakouts`**: no HTTP, DB, filesystem or logging.
`score_symbol` takes a `pd.DataFrame` of daily bars (the `read_bars` shape -- see
`app.scan.indicators`) plus an already-looked-up `iv30: float | None`; `app.api.scan` is where
the IV lookup itself happens (the latest stored option-chain snapshot, read the same way
`app.api.report` does), so this module never touches Parquet or Postgres and never decides
...
#: over 126 bars" -- the same ~6-month window `app.scan.breakouts.DEFAULT_LOOKBACK` uses for
...
class TrendComponents:
    ...
    ranked against nothing). Every indicator field is `None` exactly when `app.scan.indicators`
    ...
    `app.scan.breakouts.BreakoutEvent.follow_through_atr` already translates its own `NaN`
    ...
    IV history within a chain that exists" into one `None`, matching `app.gex.report.iv_regime`
    's own contract).
    """
...
def score_symbol(bars: pd.DataFrame, iv30: float | None) -> TrendComponents:
    ...
    any particular lookback) -- `app.scan.indicators.adx`/`efficiency_ratio`/`choppiness`/
    ...
    `app.scan.indicators`'s module docstring), so this function is what slices `bars` down to
    ...
    `iv30` is supplied by the caller (`app.api.scan`, following `api/report.py`'s snapshot
    lookup pattern) rather than looked up here -- this module stays pure, exactly like
    `app.gex.engine` (CLAUDE.md invariant 1) and exactly the caller/pure split T43 already used
    for its own calendar dependency (`app.jobs.calendar` stays out of `app.scan.breakouts` the
