...
* **Insufficient history is `NaN`, never a fabricated `0.0` or a silently shortened window.**
  ...
  dropping rows or padding with a plausible-looking number. `app.scan.trend.score_symbol`
  converts a trailing `NaN` to a plain `None` at the dataclass boundary the same way
  `app.scan.breakouts.BreakoutEvent` already does for `follow_through_atr`.
...
* **`variance_ratio` returns a `(float | None, float | None)` tuple computed once over the
  ...
  convention, for the same reason `app.scan.breakouts.summarize`'s `lookback` is caller-supplied
  ...
  there is for `adx`/`efficiency_ratio`/`choppiness`/`realized_vol`. `app.scan.trend
...
def variance_ratio(bars: pd.DataFrame, q: int = VR_Q) -> tuple[float | None, float | None]:
    ...
    **Not a rolling `Series`** -- see the module docstring's "Deviations" section for why. This
    ...
    `app.scan.breakouts.detect_events`'s `n`/`k` windows are the caller's choice, not this
    module's.
