...
**Pure, exactly like `app.gex.engine` and `app.scan.breakouts`/`app.scan.trend`**: no HTTP, DB,
...
`app.storage.flows_repository`) and returns a plain `DataFrame`; `app.api.scan` is where the
repository reads happen, the same caller/pure split every other `app.scan.*` module uses.
...
def compute_flows(
    ...
    Args:
        so_frame: Wide shares-outstanding frame, index = date ascending, one column per
            symbol -- the `app.storage.flows_repository.read_universe_shares_outstanding`
