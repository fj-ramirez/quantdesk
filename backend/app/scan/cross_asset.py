...
20-day moves. **Pure, exactly like `app.gex.engine`, `app.scan.indicators` and
`app.scan.rotation`**: no HTTP, DB, filesystem or logging anywhere in this module. Every
function takes `pd.Series`/`pd.DataFrame` closes already fetched by the caller
(`app.storage.bars_repository.read_universe_closes`'s wide shape) and returns a plain value, a
`(value, n)` pair, or the frozen `CrossAssetRow`; `app.api.scan` is the only place in this
...
-----------------------------------------------------------------------
Same house rule `app.scan.rotation`'s module docstring establishes and `app.scan.rotation`
...
calling convention `app.api.scan.get_rotation` already uses for `app.scan.rotation`), and
...
posture `app.scan.rotation.rrg_approx`'s own `benchmark.reindex(prices.index)` documents.
...
`app.scan.indicators`'s module docstring states for its own rolling indicators, extended here
...
`app.scan.trend._percentile_ranks` already uses for its own (cross-sectional) percentiles --
...
* **VRP** = `VIX - SPY RV20`, both annualized, in vol points (e.g. `18.2`, not `0.182`) -- Cboe
  already publishes VIX in vol points; `app.scan.indicators.realized_vol` returns a fraction,
...
* **Sector correlation** = mean of the pairwise 20-day correlation of daily log returns across
  the 11 sector ETFs (`app.scan.groups.SECTORS`, supplied by the caller as a wide closes
  frame -- this module has no built-in universe knowledge, matching `app.scan.rotation`'s own
...
from app.scan.indicators import RV_PERIOD, realized_vol
...
def percentile_252(
    ...
    Returns:
        ...
        `app.scan.trend._percentile_ranks` uses for its own (cross-sectional) percentiles --
...
def vrp(vix_close: float, spy_rv20: float) -> float:
    ...
    Args:
        ...
        spy_rv20: SPY's annualized 20-day realized vol as a **fraction**
            (`app.scan.indicators.realized_vol`'s own units, e.g. `0.182` for 18.2%) -- this
...
def sector_correlation(
    ...
    Args:
        closes: Wide closes frame, one column per sector ETF, one row per date, ascending --
            `app.storage.bars_repository.read_universe_closes`'s own shape. Log returns are
        ...
        window: Trailing number of *daily* rows to compute the correlation over, before
            alignment. Must be `>= 2`.
    ...
    with np.errstate(invalid="ignore", divide="ignore"):
        log_returns = np.log(numeric / numeric.shift(1))
...
def _last_finite(series: pd.Series) -> float | None:
    ...
    scan module (`app.api.scan`, `app.scan.trend`) defines privately for itself rather than
...
def _n_day_return(closes: pd.Series, n: int) -> float | None:
    ...
    `app.api.scan._return_5d`'s contract, generalized to any `n` and to a bare closes `Series`
...
class CrossAssetRow:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        `app.scan.breakouts.BreakoutEvent`/`app.scan.trend.TrendComponents` already use, so
        `app.api.scan`'s response model can build itself with `cls(**row.to_dict())`."""
        return asdict(self)
...
def compute_cross_asset_row(
    *,
    ...
    `app.storage.bars_repository.read_universe_closes` frame -- see the module docstring's
    ...
    Args:
        ...
        spy_close: SPY closes, ascending by date -- feeds `spy_rv20` (via
            `app.scan.indicators.realized_vol`) and therefore `vrp`.
    ...
    spy_rv20_series = realized_vol(pd.DataFrame({"close": spy_close}))
