...
Mirrors `app.jobs.capture`'s shape deliberately: one call per symbol funnels through a
...
job it shares a process with. See `app.jobs.scheduler`'s `update_bars_job` registration for how
...
from app.storage.bars_repository import last_bar_date, upsert_bars

__all__ = ["BarUpdateResult", "update_bars_job", "update_one_symbol"]

logger = logging.getLogger("app.jobs.bars")
...
async def update_one_symbol(
    ...
    *,
    ...
    try:
        ...
        # Same rationale as app.jobs.capture.capture_snapshot's identical catch: a provider is
    ...
    *,
    ...
    Args:
        ...
        registry: Defaults to a fresh `BarProviderRegistry()` built from settings, closed at
            ...
            closing it (same convention as `app.providers.base.OptionChainProvider`).
        session_factory: Defaults to `app.storage.bars_repository.get_session_factory` (real
