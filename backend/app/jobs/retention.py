...
that `snapshots.parquet_path` points at, by the same `app.gex.store.compute_and_store` that
wrote it -- which already does delete-then-insert, so a rebuild is idempotent -- or wholesale
via `uv run python -m app.gex.backfill`. Pruning a day therefore loses nothing permanently. It
...
from app.models.db import GexByStrike, Snapshot

__all__ = ["PruneResult", "prune_intraday_strike_detail"]

logger = logging.getLogger("app.jobs.retention")
