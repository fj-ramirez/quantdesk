...
* `GET /gex/{underlying}/latest` and `GET /gex/{underlying}/snapshots/{id}` need
  `GexResult.profile`, which T09 never persists (`app.gex.store`'s own docstring: only the
...
def _canonical_underlying(raw: str) -> str:
    ...
    `app.api.snapshots.trigger_capture`: an unknown symbol is a client error, not worth a
...
def _parse_filter(raw: str) -> ExpiryFilter | list[dt.date]:
    """Parse a `filter` query value into what `app.gex.engine.compute_all` accepts: an
    `ExpiryFilter` member, or an explicit list of dates for the `EXPIRIES:<date>[,<date>...]`
    form `app.gex.engine._filter_label` produces (and this parses back) -- outside the enum
...
def _parse_history_filter(raw: str) -> ExpiryFilter:
    ...
    `app.gex.store.compute_and_store` actually persisted (by default: `ALL`, `ZERO_DTE`,
    `EX_ZERO_DTE` -- `app.gex.store.DEFAULT_FILTERS`). An explicit `EXPIRIES:...` filter is
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                f"persisted by app.gex.store, so they cannot be queried here)"
            ),
        ) from None
