"""Read API for the decision engine (T60) -- `app.scan.decisions` behind HTTP.

Routes, both computing on request (no new table, the same posture every `app.api.scan` route
takes for a single-user app):

* `GET /api/decisions?filter=&min_score=` -- one `DecisionOut` per `Underlying` member that has
...
* `GET /api/decisions/{underlying}?filter=` -- one symbol. 422 on an unknown symbol, and the
  same clean 404 with a specific `detail` `app.api.gex` uses when a symbol has never been
  captured (the contract the frontend's empty state depends on, T37).

Every input is built by `app.api.scan.build_regime_rows` -- the exact pipeline `/regime` runs,
...
pipeline already fetched, with `app.scan.breakouts`' plan defaults (`N=20`, `k=5`, 126-bar
...
from app.api.gex import _canonical_underlying
from app.api.scan import _validate_regime_filter, build_regime_rows
from app.gex.engine import ExpiryFilter
from app.jobs.decisions import DecisionsJobResult, decide_build, record_decisions_job
from app.scan.decisions import DecisionResult
from app.scan.outcomes import summarize_outcomes
...
class OpportunityOut(BaseModel):
    """Mirrors `app.scan.decisions.Opportunity`; see that dataclass and the module docstring
...
class DecisionRecordOut(BaseModel):
    ...
    `app.storage.decisions_repository.DecisionRecord`."""
...
def get_symbol_decisions(
    ...
    build = builds[0]
    if build.row is None:
        # Same `detail` string `app.api.gex._get_latest_row` raises: the frontend's empty
