...
`POST /api/snapshots/capture` runs the exact same `app.jobs.capture.capture_snapshot` the
...
from app.jobs.capture import capture_snapshot, get_session_factory
...
class CaptureResponse(BaseModel):
    """Mirrors `app.jobs.capture.CaptureResult` -- see that dataclass for field meanings."""
...
async def trigger_capture(
    ...
    `CaptureResult.error` from `app.jobs.capture` becomes the response detail either way, so
