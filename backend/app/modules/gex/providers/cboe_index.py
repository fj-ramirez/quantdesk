...
(``app.modules.gex.providers.cboe``, **not modified by this task**). Reuses that module's ``httpx``
...
independent: a bars provider (``app.modules.gex.providers.bars.BarProvider``), not an
...
the 17:30 ET bars job gets same-day data (same as `app.modules.gex.providers.yahoo`'s settled-session
...
--------------------------------------------
``app.modules.gex.models.bars.DailyBar`` declares ``open``/``high``/``low``/``close`` all required,
...
``app.modules.gex.scan.cross_asset``) must treat that range as fabricated. This is restated, not merely
cross-referenced, in ``docs/validation-scan.md``.

``^VIX`` changes hands from Yahoo to Cboe
-------------------------------------------
``T42`` already fetches ``^VIX`` from ``app.modules.gex.providers.yahoo`` (``BARS_PROVIDER=yahoo`` is the
default for every symbol not listed in a ``BAR_PROVIDER_GROUPS`` entry). Listing ``^VIX`` in
the ``cboe_index`` group (``app.core.config.settings.BAR_PROVIDER_GROUPS``) moves it here, because
``BarProviderRegistry.provider_name_for`` resolves a group entry ahead of the default
(``app.modules.gex.providers.bars``'s own module docstring). This is deliberate, not incidental: the Cboe
file is native OHLC back to 1990 and has none of Yahoo's partial-last-bar problem (see
``app.modules.gex.providers.yahoo``'s own module docstring for why that problem exists there at all).
...
``app.modules.gex.storage.bars_repository.upsert_bars``), so a re-backfill through this provider
...
* **A non-CSV 200 body** (Cboe, like its own options endpoint, can serve an HTML error/denial
  ...
  raises :class:`~app.modules.gex.providers.bars.UpstreamUnavailable`, never a bare parse exception.
...
* **A bad or unparseable row** (a blank line, a row with a non-numeric field) is skipped and
  ...
  ``app.modules.gex.providers.cboe``'s unknown-root policy and ``app.modules.gex.providers.yahoo``'s null-OHLC skip
  both already use.
* **Volume.** These CSVs carry no volume column at all; every ``DailyBar`` here has
  ...
  discipline, the "no field at all" case named in ``app.modules.gex.models.bars.DailyBar``'s own
...
# agent -- same browser-like string as app.modules.gex.providers.cboe, same rationale.
_USER_AGENT = (
...
def _parse_date(raw: str) -> dt.date:
    ...
    `app.modules.gex.providers.etf_flows`'s own date-only parse: this is a calendar date with no time
    component to attach a zone to -- see `app.modules.gex.models.bars.DailyBar`'s own docstring for why
...
class CboeIndexHistoryProvider(BarProvider):
    ...
    async def _fetch_csv(self, url: str, *, requested_symbol: str) -> str:
        ...
        Same retry shape as `app.modules.gex.providers.cboe._fetch_json`: every non-2xx status retries
...
if __name__ == "__main__":
    ...
    if len(sys.argv) not in (2, 3):
        print("usage: python -m app.modules.gex.providers.cboe_index SYMBOL [years]", file=sys.stderr)
        raise SystemExit(2)
