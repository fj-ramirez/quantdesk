...
returns as a :class:`~app.modules.gex.models.chain.ChainSnapshot`, so correctness here matters more than
speed.
...
* **``timestamp`` is payload-generation time, not data-effective time (T34).** Verified
  ...
  :class:`~app.modules.gex.models.chain.ChainSnapshot`'s called that field "effective time of the data",
  ...
  :func:`app.modules.gex.jobs.calendar.effective_data_time` rather than display ``captured_at`` directly.
...
``underlying_for_root`` (in ``app.modules.gex.models.chain``) raises ``ValueError`` for any root this
...
# `data.options`, single root == ticker). See `app.modules.gex.models.chain.Underlying` for the per-symbol
...
def _to_int(value: Any) -> int | None:
    ...
    ``app/modules/gex/models/chain.py`` warns about. Rounding is the smaller lie of the two.
    """
...
def _contract_from_vendor(raw: dict[str, Any]) -> OptionContract:
    ...
    Raises:
        ValueError: the OCC symbol does not parse, or its root is not one this application
            maps to an :class:`Underlying` (see :func:`app.modules.gex.models.chain.underlying_for_root`).
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print(
            "usage: python -m app.modules.gex.providers.cboe SPX|SPY|QQQ|GLD|DIA|<extended symbol>",
            file=sys.stderr,
        )
        raise SystemExit(2)
