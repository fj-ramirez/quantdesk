...
PLAN.md §1 names this the fallback for the Cboe delayed-quotes feed (``app.modules.gex.providers.cboe``):
...
Run ``uv run python -m app.modules.gex.providers.marketdata SPX`` (with ``MARKETDATA_TOKEN`` set) to get
...
standard OCC symbol (e.g. ``"AAPL271217C00100000"``), so :func:`app.modules.gex.models.chain.parse_occ_symbol`
...
needed here (contrast :data:`app.modules.gex.providers.cboe._VENDOR_SYMBOL`); the canonical
...
comes from ``MARKETDATA_TOKEN`` in ``app.core.config.settings``; a missing token raises
...
Unknown-root policy
--------------------
Same as ``app.modules.gex.providers.cboe``: skip and log the single contract rather than failing the whole
    ...
    def _parse_payload(self, underlying: Underlying, payload: dict[str, Any]) -> ChainSnapshot:
        ...
        for i in range(len(symbols)):
            try:
                ...
                # rejects. Same skip-and-log policy as app.modules.gex.providers.cboe, and deliberately
        ...
        if not contracts:
            # Same reasoning as app.modules.gex.providers.cboe: a zero-contract snapshot stored as a
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("usage: python -m app.modules.gex.providers.marketdata SPX|SPY|QQQ|GLD|DIA", file=sys.stderr)
        raise SystemExit(2)
