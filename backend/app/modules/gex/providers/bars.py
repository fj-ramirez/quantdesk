...
Mirrors `app.modules.gex.providers.base`: `BarProvider` is the one abstract class every bars data source
implements, and every caller (the update job, the backfill CLI, the `/api/gex/bars` route) goes
...
to bars exactly as it does to the option-chain providers in `app.modules.gex.providers.__init__`.

The error hierarchy is reused, not reinvented: `app.modules.gex.providers.base.ProviderError`,
...
They are re-exported here so `app.modules.gex.providers.yahoo` / `app.modules.gex.providers.tiingo` can import them
from the bars module they already depend on, rather than reaching into `app.modules.gex.providers.base`
...
``"cboe_index:^VIX,^VIX9D,^VIX3M,^VIX6M,^VVIX,^SKEW"`` (`app.modules.gex.providers.cboe_index`).

Concrete provider modules (`app.modules.gex.providers.yahoo`, `app.modules.gex.providers.tiingo`) are imported lazily,
...
class BarProvider(ABC):
    ...
    Same lifecycle contract as `app.modules.gex.providers.base.OptionChainProvider`: cheap to construct,
    safe to reuse across calls, owns its own `httpx.AsyncClient` lazily with an optional
    injected one for tests (see `app.modules.gex.providers.cboe.CboeProvider` for the exact pattern every
    ...
    async def fetch_daily_bars(
        ...
        Args:
            symbol: A plain ticker as this app's callers know it (`'SPY'`, `'^VIX'`, `'SPX'`).
                ...
                -- see `app.modules.gex.providers.yahoo`'s module docstring -- never the caller's, and never
            ...
            start: Inclusive lower bound, exchange-local date.
                ...
                in-progress trading session -- see `app.modules.gex.providers.yahoo`'s module docstring for
...
def get_bar_provider(symbol: str, *, default: str | None = None, groups: str | None = None) -> BarProvider:
    ...
    only ever need one symbol at a time, e.g. a single `/api/gex/bars/{symbol}` request.
    """
