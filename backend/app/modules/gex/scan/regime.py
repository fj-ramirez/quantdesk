...
**Pure, exactly like `app.modules.gex.gex.engine`, `app.modules.gex.scan.breakouts` and `app.modules.gex.scan.trend`**: no HTTP, no
database, no filesystem, no logging. In particular this module does **not** import
`app.modules.gex.jobs.calendar`, for the same reason `app.modules.gex.scan.breakouts`'s own docstring gives for the same
...
clock -- `app.modules.gex.api.scan` calls `app.modules.gex.jobs.calendar.effective_data_time` (T34) once per symbol and
...
* `app.modules.gex.gex.engine.KeyLevels` -- the persisted `gex_levels` row, already the exact shape this
  ...
  `app.modules.gex.api.scan._load_regime_inputs` for how the caller reconstructs one without opening
  Parquet.
* `app.modules.gex.gex.engine.StrikeGex` -- the persisted `gex_by_strike` rows, for the by-strike scans
  (room beyond, 0DTE share) below.

**Deliberately not `app.modules.gex.gex.engine.GexResult` itself**, even though the task's own plan
...
per `(snapshot, filter, strike)` for every filter `app.modules.gex.gex.store.DEFAULT_FILTERS` computes --
...
and `app.modules.gex.api.scan._load_gex_inputs`'s own docstring say this endpoint must not do -- and the
...
Gated exactly like `app.modules.gex.gex.report.dealer_positioning`: below `POSITIONING_RATIO_FLOOR`
...
`app.modules.gex.api.scan` computes `chain_age_minutes` -- how many minutes before its trading day's close
...
from app.modules.gex.gex.report import (
...
def _f(x: Any) -> float | None:
    ...
    `app.modules.gex.gex.engine._f` and `app.modules.gex.gex.report._f` -- each pure module keeps its own copy rather
...
class RegimeRow:
    ...
    result" discipline `app.modules.gex.gex.engine.SnapshotMeta` and `app.modules.gex.gex.report.ReportResult`
    already follow.

    `verdict` is `None` in exactly two cases, both spelled out in `reasons`: the chain is
    noise-dominated (`positioning.noise_dominated`, the same gate `app.modules.gex.gex.report
...
def _nearest_walls(
    ...
    *above* spot, per `app.modules.gex.gex.report.LevelSet`'s docstring), so this picks by position, not by
...
def compute_regime_row(
    *,
    ...
    Args:
        underlying: Echoed onto the result. Not validated against `app.modules.gex.models.chain.Underlying`
            here -- that is `app.modules.gex.api.scan`'s job, the same split `app.modules.gex.gex.report`'s own module
        ...
        atr14: Daily-bar ATR(14), or `None` if unmeasurable (`app.modules.gex.scan.indicators.atr`'s own
            `NaN` translated to `None` at the caller's dataclass boundary).
        iv30: ATM ~30-day implied vol off the same snapshot (`app.modules.gex.gex.report.iv_regime`'s
            `atm_iv`), or `None`.
        ...
        effective_at: `app.modules.gex.jobs.calendar.effective_data_time(captured_at, delayed_minutes)` --
        ...
        ratio_floor: Forwarded to `app.modules.gex.gex.report.dealer_positioning`. Defaults to
    ...
    *,
    ...
    if zero_dte_known:
        ...
        zero_dte_reason = (
            ...
            "payload by capture time -- see app.modules.gex.scan.regime module docstring); fade evaluated "
