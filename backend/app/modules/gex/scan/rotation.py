...
**Pure, exactly like `app.modules.gex.gex.engine`, `app.modules.gex.scan.breakouts` and `app.modules.gex.scan.trend`**: no HTTP, DB,
filesystem or logging anywhere in this module. Every function takes `pd.DataFrame`/`pd.Series`
prices already fetched by the caller (`app.modules.gex.storage.bars_repository.read_bars` /
`read_universe_closes`'s shape) and returns a `pd.DataFrame` or a frozen dataclass; `app.modules.gex.api
...
reason, and `app.modules.gex.api.scan`'s `/rotation` response carries the same disclaimer forward as an
...
history is `NaN`, never a fabricated number" discipline `app.modules.gex.scan.indicators` already applies,
...
* **Breadth is coarse and says so.** `sector_breadth` computes only what a ~50-symbol universe
  ...
  of this app's budget) -- the caller (`app.modules.gex.api.scan`) is responsible for the "sector-level
  breadth" label the plan requires on the outside of this dataclass; this module only computes
  the numbers.
* **Percentile ties / warm-up conventions already established by `app.modules.gex.scan.trend` are not
...
def _to_float(series: pd.Series) -> pd.Series:
    ...
    *raises* `TypeError` in this pandas version (`float(pandas.NA)` is not a supported
    ...
    a symbol with zero stored bars (`app.modules.gex.api.scan.get_rotation`'s `XLF`-with-no-bars case) --
...
def weekly_closes(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        daily: Wide frame, one column per symbol, index of dates (either a `DatetimeIndex`
            already, or anything `pandas.to_datetime` accepts elementwise -- `app.modules.gex.storage
            ...
            below normalizes to `NaN` the same way every other `app.modules.gex.scan` module treats a
            missing reading.
...
def _rolling_zscore(series: pd.Series, w: int) -> pd.Series:
    ...
    implemented via the same pandas `min_periods` mechanism `app.modules.gex.scan.indicators` already uses
    ...
    variation to deviate from") instead, the same `0/0` guard pattern `app.modules.gex.scan.indicators.adx`
...
def _last_finite_pair(a: pd.Series, b: pd.Series, n: int) -> float:
    ...
    the same one `app.modules.gex.scan.indicators` uses -- only the frozen-dataclass-returning
    ...
    `app.modules.gex.scan.breakouts.BreakoutSummary.last_event` already gives for an unrelated reason
...
def relative_returns(
    ...
    Args:
        prices: Wide **daily** close frame (`app.modules.gex.storage.bars_repository.read_universe_closes`'s
        ...
        windows: Trading-day windows. Plan default `(5, 20, 65)`, approximating 1/4/13 weeks --
            ...
            function enforces no menu (the API layer's job, mirroring how `app.modules.gex.scan.breakouts
...
class SectorBreadth:
    ...
    caller (`app.modules.gex.api.scan`) is responsible for carrying that "sector-level breadth" label
    ...
    `len(app.modules.gex.scan.groups.SECTORS)`, normally 11) so a reader can tell "4 of 11 are above" from
    ...
    negative reading" discipline `app.modules.gex.scan.trend.TrendComponents` already applies to a missing
    indicator.
    ...
    `app.modules.gex.scan.trend.TrendComponents.iv_rv_ratio` already has relative to `rv20`/`iv30`.
    """
...
def _last_finite(series: pd.Series) -> float | None:
    ...
    only `series.iloc[-1]`, matching `app.modules.gex.scan.trend`'s own private helper of the same name and
    exact behavior (not imported from there: a two-line, module-private detail each pure
    `app.modules.gex.scan` module keeps to itself, the same reasoning `app.modules.gex.scan.indicators.true_range`
    gives for staying public rather than folded away). For a single symbol's own rolling
    indicator (`app.modules.gex.scan.trend`'s use) the series' last row is `NaN` only during genuine
...
def sector_breadth(prices: pd.DataFrame, sectors: tuple[str, ...] | None = None) -> SectorBreadth:
    ...
    Args:
        ...
        sectors: Defaults to `app.modules.gex.scan.groups.SECTORS` (the 11 SPDR sector ETFs). Not imported
            at module level and instead defaulted here via `None` so this module has no import
            dependency on `app.modules.gex.scan.groups` merely to know its own default -- a caller that
            already has the tuple (the ordinary case, `app.modules.gex.api.scan`) passes it in; a test
    ...
    Returns:
        `SectorBreadth`.
    """
    from app.modules.gex.scan.groups import SECTORS as _default_sectors
