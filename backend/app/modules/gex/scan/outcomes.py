...
**Pure**, on the same contract as every other `app.modules.gex.scan` module: no HTTP, no database, no
filesystem, no logging, no clock. The caller (`app.modules.gex.jobs.decisions`) fetches the bars and
...
class TradeSpec:
    """The four numbers an evaluation needs; a projection of `app.modules.gex.scan.decisions.Opportunity`."""
...
class GroupStats:
    ...
    trades, for the same reason `app.modules.gex.scan.breakouts.summarize` withholds a rate on fewer
