...
1. **Record.** Run the `/decisions` pipeline (`app.modules.gex.api.scan.build_regime_rows` +
   `app.modules.gex.scan.decisions.decide`) and insert every emitted opportunity through
...
-- see `app.modules.gex.api.scan`'s T45 timing note), so it runs in a worker thread via
...
from app.modules.gex.scan.decisions import DecisionResult, decide
from app.modules.gex.scan.outcomes import evaluate
...
def breakout_summary_for(build: RegimeBuild) -> BreakoutSummary | None:
    """`app.modules.gex.scan.breakouts.summarize` over the plan-default window, from the bars the regime
    ...
    *,
    ...
    Args:
        ...
        bars_session_factory: For `daily_bars`. Same default/injection story. The `/decisions`
            pipeline itself reads through `app.modules.gex.api.scan`'s module-level factories, which tests
