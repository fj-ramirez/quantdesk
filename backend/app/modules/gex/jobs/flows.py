...
Mirrors `app.modules.gex.jobs.bars`'s shape (one call per unit of work funnels through a result dataclass
...
**P0 guardrail, same as `app.modules.gex.jobs.bars.bars_update_job`'s own docstring states for itself: this
...
logger = logging.getLogger("app.modules.gex.jobs.flows")
...
def _default_providers() -> list[SharesOutstandingProvider]:
    ...
    SPDR and iShares, plus T59's VanEck, Invesco and USCF. `app.modules.gex.providers.etf_flows.
    ...
    *,
    ...
    Args:
        ...
        providers: Defaults to `_default_providers()` (fresh SPDR + iShares provider
            ...
            keeps ownership of closing them (same convention as `app.modules.gex.jobs.bars.update_bars_job`
            and its `registry` parameter).
        session_factory: Defaults to `app.modules.gex.storage.flows_repository.get_session_factory` (real
    ...
    try:
        for provider in active_providers:
            ...
            try:
                ...
                # Same rationale as app.modules.gex.jobs.bars.update_one_symbol's identical catch: a
