...
`app/modules/gex/jobs/scheduler.py`'s `misfire_grace_time=None` only rescues a run whose *process was
...
* `app/modules/gex/jobs/scheduler.py` registers a 20:00 NY "safety net" cron that calls the exact same
...
logger = logging.getLogger("app.modules.gex.jobs.catchup")

# `settings.TZ` (default "America/New_York"), same rationale as `app/modules/gex/jobs/scheduler.py`: the
...
# The scheduled EOD job fires at 16:20 NY (`app/modules/gex/jobs/scheduler.py`). Cboe keeps serving the
...
def last_completed_trading_day(now: dt.datetime) -> dt.date:
    ...
    the health endpoint's staleness rule (`app/modules/gex/api/health.py`), so both use one definition of
...
async def catch_up_missed_eod(
    ...
    *,
    ...
    Returns:
        ...
        already degrades to `CaptureResult(ok=False, ...)` (see `app.modules.gex.jobs.capture`), and this
    ...
    # accepting connections. `asyncio.to_thread` (same pattern `app/modules/gex/jobs/capture.py` already
...
async def startup_catchup_job() -> None:
    ...
    `capture_eod_job` in `app/modules/gex/jobs/scheduler.py`, this wraps its own body in try/except: an
