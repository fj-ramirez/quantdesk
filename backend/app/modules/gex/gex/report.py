...
per-strike GEX aggregates :mod:`app.modules.gex.gex.engine` already produces.

**This module is pure**, on exactly the same contract as :mod:`app.modules.gex.gex.engine` and
:mod:`app.modules.gex.gex.greeks`: no HTTP, no database, no filesystem, no logging, and deterministic
given its inputs. That is what lets the read API (`app.modules.gex.api.report`), a future capture-time
pre-computation, and an offline backtest all share one implementation. The clock is never
read here either — every "as of" instant comes from the :class:`~app.modules.gex.gex.engine.GexResult`
...
* a :class:`~app.modules.gex.gex.engine.GexResult` — spot, walls, flip point, top strikes, net/abs GEX;
* the ``to_frame`` DataFrame — per-contract ``open_interest``, ``volume``, ``iv``, ``bid``,
  ``ask``, ``dte``, which the aggregates have already summed away.

``filters`` re-applies :func:`~app.modules.gex.gex.engine.expiry_mask` to the frame so that every figure
...
from app.modules.gex.gex.engine import (
    DEFAULT_IV_POLICY,
    ExpiryFilter,
    GexResult,
    IvPolicy,
...
def _f(x: Any) -> float | None:
    ...
    Deliberately mirrors ``app.modules.gex.gex.engine._f`` rather than importing it: that one is private
...
class ReportResult:
    ...
    genuinely absent — on the same terms as :meth:`app.modules.gex.gex.engine.GexResult.to_dict`, so the
...
def build_report(
    ...
    *,
    ...
    Args:
        result: The output of :func:`app.modules.gex.gex.engine.compute_all` for this snapshot and filter.
