...
Run as::
    ...
    uv run python -m app.modules.gex.flows_fetch [--symbols XLK,IWM]

Mirrors `app.modules.gex.bars_backfill`'s shape (top-level module rather than nested under `app.modules.gex.jobs`,
...
Every run just calls `app.modules.gex.jobs.flows.update_flows_job` once.
...
per `app.modules.gex.storage.flows_repository.insert_new_rows`'s `(symbol, date)`-keyed insert-once
...
from app.modules.gex.storage.flows_repository import get_session_factory

__all__ = ["main"]

logger = logging.getLogger("app.modules.gex.flows_fetch")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "covers (app.modules.gex.providers.etf_flows.ALL_SUPPORTED_SYMBOLS)"
        ),
    )
    ...
    # as app.modules.gex.bars_backfill.main's identical call.
    get_session_factory()
