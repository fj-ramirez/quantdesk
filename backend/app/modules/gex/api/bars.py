...
`GET /api/gex/bars/{symbol}?start=&end=` and `GET /api/gex/universe` exist for the six continuation-plan
...
`app.modules.gex.api.chains` or `app.modules.gex.api.snapshots`) because bars have no relationship to option chains or
...
from app.modules.gex.jobs.intraday_bars import session_daily_bar
...
class IntradayBarsResponse(BaseModel):
    """`GET /api/gex/bars/{symbol}/intraday`: the stored series plus the derived session bar."""
...
class BarOut(BaseModel):
    """Mirrors `app.modules.gex.models.bars.DailyBar` -- see that model's docstring for field meanings,
...
def get_universe() -> UniverseResponse:
    ...
    page reads bars for. A thin read of `app.core.config.settings`, not the database: this route
    answers "what symbols does this app track," not "which of them have data yet" (that's what
    an empty list from `/api/gex/bars/{symbol}` communicates per-symbol).
    """
...
def get_bars(
    ...
    /api/gex/bars/%5EVIX` and a client that sends the literal `^` both arrive here as `"^VIX"`; see
    ...
    "clean empty state, not a synthetic error" contract `app.modules.gex.api.chains` uses for an unlisted
    expiry.
    """
...
def get_intraday_bars(
    ...
    Same "empty is not an error" contract as `GET /api/gex/bars/{symbol}` above: a symbol outside
    ...
    `daily_bars` -- see `app.modules.gex.jobs.intraday_bars.session_daily_bar` for why that separation is
