"""Read API for the options-intelligence report (T39) -- `app.modules.gex.gex.report` behind HTTP.

Shape and posture are deliberately identical to `app.modules.gex.api.gex`: same underlying validation
...
`?format=text` returns `app.modules.gex.gex.report.render_text` output as `text/plain` so the report exists
...
# Imported from `app.modules.gex.api.gex` rather than reimplemented: these four are the *contract* for
...
from app.modules.gex.api.gex import (
    _canonical_underlying,
    _get_latest_row,
    _parse_filter,
    _snapshot_meta,
)
from app.modules.gex.api.schemas import ReportOut
from app.modules.gex.gex.engine import ExpiryFilter, compute_all, to_frame
from app.modules.gex.gex.report import CFD_INSTRUMENTS, build_report, render_text
...
def get_report(
    ...
    non-numeric value with a 422 before it ever reaches `app.modules.gex.gex.report` -- there is no path by
