"""`GET /api/gex/symbols` -- the core/extended universe split, at the wire (T47).

A reader of `/api/gex/snapshots` (or any other per-underlying endpoint) cannot tell a core
...
validated-at-startup lists (see `app/core/config.py`), so this route does no work beyond echoing
...
class SymbolsResponse(BaseModel):
    ...
    #: this endpoint or by anything reading it -- see `app/core/config.py`'s `SYMBOLS` docstring.
    core: list[str]
