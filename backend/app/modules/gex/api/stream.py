"""`GET /api/gex/stream/{underlying}` -- Server-Sent Events for level updates (TASKS.md T19).
...
down this channel would duplicate `GET /api/gex/gex/{underlying}/latest`'s contract in a second
...
logger = logging.getLogger("app.modules.gex.api.stream")
...
async def stream_levels(underlying: str, request: Request) -> StreamingResponse:
    ...
    and `GET /api/gex/stream/SPXX` should say so in the ordinary way.
    """
    try:
