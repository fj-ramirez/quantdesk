...
One :class:`~app.modules.gex.models.chain.ChainSnapshot` == one Parquet file: one row per
:class:`~app.modules.gex.models.chain.OptionContract`, with the snapshot-level fields (``underlying``,
...
the write- and read-side halves of making ``Snapshot.parquet_path`` (`app.modules.gex.models.db`) mean
...
#: `app/modules/gex/models/chain.py` on why that distinction has to survive storage intact.
...
def to_data_dir_relative_path(path: str | Path, data_dir: str | Path | None = None) -> str:
    ...
    `Snapshot.parquet_path` (see that column's docstring in `app.modules.gex.models.db`).
